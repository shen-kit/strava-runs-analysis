...
def _totals_rows(acts: list[Activity], bucket: str):
    ...
    for a in acts:
        k = bucket_key(a.local_date, bucket)
        g = groups[k]
        g["bucket"] = k
        g["run_count"] += 1
        g["distance_m"] += activity_distance_m(a)
        g["moving_time_s"] += activity_duration_s(a)
        g["elevation_gain_m"] += a.elevation_gain_m or 0
        g["days_run"].add(a.local_date.isoformat())
    rows = []
    for k in sorted(groups):
        g = groups[k]
        rows.append({**g, "days_run": len(g["days_run"])})
    return rows
...
def summary(session: Session = Depends(get_session)):
    ...
    return {
        ...
        "average_pace_s_per_km": move / (dist / 1000)
        if dist > 0 and move > 0
        else None,
        "longest_run_distance_m": max(
            [activity_distance_m(a) for a in acts], default=0
        ),
        "latest_activity_date": max([a.local_date for a in acts], default=None),
        "current_month_distance_m": sum(
            activity_distance_m(a)
            for a in acts
            if a.local_date.year == today.year and a.local_date.month == today.month
        ),
        "current_year_distance_m": sum(
            activity_distance_m(a) for a in acts if a.local_date.year == today.year
        ),
    }
...
def totals(
    ...
    return [
        {k: v for k, v in r.items() if k != "days_run"}
        for r in _totals_rows(get_activities(session), bucket)
    ]
...
def weekly_volume(session: Session = Depends(get_session)):
    ...
    for i, r in enumerate(rows):
        ...
        r["rolling_4_week_avg_distance_m"] = (
            sum(w["distance_m"] for w in window) / len(window) if window else None
        )
    return rows
...
def consistency(session: Session = Depends(get_session)):
    ...
    return {
        "weeks": rows,
        "current_week_count": next(
            (r["run_count"] for r in rows if r["bucket"] == current_week), 0
        ),
        "average_runs_per_week": sum(r["run_count"] for r in nonzero) / len(nonzero)
        if nonzero
        else 0,
    }
    ...
    return [
        {
            ...
            "pace_s_per_km": (
                ...
                if r["distance_m"] > 0 and r["moving_time_s"] > 0
                else None
            ),
        }
        for r in rows
...
def elevation(
    ...
    return [
        {"bucket": r["bucket"], "elevation_gain_m": r["elevation_gain_m"]}
        for r in _totals_rows(get_activities(session), bucket)
    ]
...
def best_effort_trend(distances: str = "", session: Session = Depends(get_session)):
    targets = (
        {float(x) for x in distances.split(",") if x.strip()}
        if distances
        else {row.distance_m for row in get_enabled_best_effort_distances(session)}
    )
    rows = session.exec(
        select(BestEffort, Activity)
        .join(Activity, Activity.id == BestEffort.activity_id)
...
def distance_distribution(session: Session = Depends(get_session)):
    buckets = [
        ...
        ("5-10km", 5000, 10000),
        ("10-15km", 10000, 15000),
        ("15-21.1km", 15000, 21097.5),
        ("21.1km+", 21097.5, float("inf")),
    ]
    out = [
        {"bucket": name, "run_count": 0, "distance_m": 0.0} for name, _, __ in buckets
    ]
    for a in get_activities(session):
        d = activity_distance_m(a)
