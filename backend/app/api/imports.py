...
async def upload_strava_zip(
    ...
    extensions = {
        ...
        for x in force_reprocess_extensions.split(",")
        if x.strip()
    }
    extensions = {x for x in extensions if x in {"gpx", "fit", "tcx"}}
    background_tasks.add_task(
        process_import_job, job.id, zip_path, tmp_dir, force_reprocess_all, extensions
    )
...
async def upload_activity_files(
    ...
    if not files:
        raise HTTPException(
            status_code=400, detail="At least one activity file is required"
        )
    allowed = {".gpx", ".tcx", ".fit", ".gpx.gz", ".tcx.gz", ".fit.gz"}
    for file in files:
        if not file.filename or suffix_key(Path(file.filename)) not in allowed:
            raise HTTPException(
                status_code=400, detail=f"Unsupported activity file: {file.filename}"
            )
    settings = get_settings()
    tmp_dir = Path(settings.import_tmp_dir) / str(uuid.uuid4())
    tmp_dir.mkdir(parents=True, exist_ok=True)
    ...
    return [
        {
            ...
            for k in [
                ...
                "reprocessed_count",
                "failed_count",
                "skipped_non_run_activities_count",
                "error_message",
                "created_at",
                "started_at",
                "completed_at",
...
def get_import(import_job_id: int, session: Session = Depends(get_session)):
    ...
    rows = session.exec(
        select(ActivityImportDiagnostic)
        .where(ActivityImportDiagnostic.import_job_id == import_job_id)
        .order_by(ActivityImportDiagnostic.id)
    ).all()
    out["diagnostics"] = [
        {
