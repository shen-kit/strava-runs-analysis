...
def activity_summary(a: Activity) -> dict:
    return {
        "id": a.id,
        "title": a.title,
        "source_sport_type": a.source_sport_type,
        "start_time_utc": a.start_time_utc,
        "start_time_local": a.start_time_local,
        "local_date": a.local_date,
        "timezone": a.timezone,
        "source_distance_m": a.source_distance_m,
        "computed_distance_m": a.computed_distance_m,
...
def list_activities(
    ...
    if year:
        q = q.where(
            Activity.local_date >= f"{year}-01-01",
            Activity.local_date <= f"{year}-12-31",
        )
    if date_from:
        q = q.where(Activity.local_date >= date_from)
    if date_to:
        q = q.where(Activity.local_date <= date_to)
    rows = session.exec(q).all()

    def dist(a: Activity):
        return (
            a.source_distance_m
            if a.source_distance_m is not None
            else (a.computed_distance_m or 0)
        )

    def dur(a: Activity):
        return (
            a.moving_time_s if a.moving_time_s is not None else (a.elapsed_time_s or 0)
        )
    ...
    def sort_key(a: Activity):
        ...
        if sort == "pace":
            return (
                a.avg_pace_s_per_km if a.avg_pace_s_per_km is not None else float("inf")
            )
        dt = a.start_time_utc or a.start_time_local
        return (
            dt.timestamp()
            if dt
            else datetime.combine(a.local_date, datetime.min.time()).timestamp()
        )
...
def delete_all_activities(session: Session = Depends(get_session)):
    ...
    if active_job:
        raise HTTPException(
            status_code=409,
            detail="Cannot delete activities while an import job is pending or processing",
        )
...
def _delete_derived(session: Session, activity_id: int) -> None:
    for cls in (ActivitySplit, BestEffort, ActivityRoute):
        for obj in session.exec(
            select(cls).where(cls.activity_id == activity_id)
        ).all():
            session.delete(obj)
    session.flush()
...
def reprocess_activity(activity_id: int, session: Session = Depends(get_session)):
    ...
    if len(rows) < 2:
        raise HTTPException(
            status_code=400,
            detail="Activity has insufficient track points to reprocess",
        )
    parsed = [
        ParsedTrackPoint(
    ...
    if len(cleaned) < 2:
        raise HTTPException(
            status_code=400,
            detail="Activity has insufficient clean track points to reprocess",
        )
    ...
    if not has_imported_summary:
        ...
        activity.avg_pace_s_per_km = (
            moving / (distance_for_pace / 1000)
            if moving and distance_for_pace
            else None
        )
    ...
    session.flush()
    for sp in splits:
        session.add(ActivitySplit(activity_id=activity_id, **sp))
    for ef in efforts:
        session.add(BestEffort(activity_id=activity_id, **ef))
    if route:
        session.add(ActivityRoute(activity_id=activity_id, **route))
    session.commit()
...
def get_route(activity_id: int, session: Session = Depends(get_session)):
    ...
    r = session.exec(
        select(ActivityRoute).where(ActivityRoute.activity_id == activity_id)
    ).one_or_none()
    if not r:
        return {
            "activity_id": activity_id,
            "simplified_points_json": [],
    ...
    return {
        "activity_id": activity_id,
        "simplified_points_json": r.simplified_points_json,
        "original_point_count": r.original_point_count,
        "simplified_point_count": r.simplified_point_count,
        "simplification_tolerance_m": r.simplification_tolerance_m,
    }
...
def get_best_efforts(activity_id: int, session: Session = Depends(get_session)):
    ...
    rows = session.exec(
        ...
        .order_by(BestEffort.distance_m)
    ).all()
    return [r.model_dump() for r in rows]
...
def get_streams(
    activity_id: int,
    types: str = "pace,heart_rate,cadence,elevation",
    session: Session = Depends(get_session),
):
    activity = activity_or_404(session, activity_id)
    ...
    pauses = [
        {
            ...
            "duration_s": b.elapsed_time_s - a.elapsed_time_s,
        }
        for a, b in zip(points, points[1:])
        if a.elapsed_time_s is not None
        and b.elapsed_time_s is not None
        and b.elapsed_time_s - a.elapsed_time_s > PAUSE_GAP_S
    ...
    return {
        "activity_id": activity_id,
        "x_axis": "distance_m",
        "x_domain_m": [0, full_distance_m],
        "streams": downsample_streams(streams),
        "pauses": pauses,
    }
...
def get_route_overlay(
    activity_id: int,
    metric: str = Query("pace", pattern="^(pace|heart_rate|gradient|cadence)$"),
    session: Session = Depends(get_session),
):
    activity = activity_or_404(session, activity_id)
    rows = session.exec(
    ...
    gps = [
        p
        for p in points
        if p.lat is not None and p.lon is not None and p.distance_m is not None
    ]
    markers = []
    pause_features = []
    if gps:
                ...
                markers.append(
                    {
                        ...
                        "gap_s": b.elapsed_time_s - a.elapsed_time_s,
                    }
                )
    ...
    return {
        ...
        "has_heart_rate": any(p.heart_rate_bpm is not None for p in rows),
        "has_cadence": any(p.cadence_spm is not None for p in rows),
        "markers": markers,
        "paused_geojson": {"type": "FeatureCollection", "features": pause_features},
