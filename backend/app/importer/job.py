...
def fields_detected(points: list[ParsedTrackPoint]) -> list[str]:
    ...
    for name in (
        ...
        if any(getattr(p, name) is not None for p in points):
            fields.append(name)
    return fields
...
def elapsed_for_points(points: list[ParsedTrackPoint]) -> list[float | None]:
    ...
    return [
        ((p.timestamp - first).total_seconds() if p.timestamp and first else None)
        for p in points
    ]
...
def diag(
    ...
    return ActivityImportDiagnostic(
        ...
        source_activity_id=meta.source_activity_id,
        source_filename=meta.filename,
        parser_name=parser_name,
        file_hash=file_hash,
        inferred_title=inferred_title,
        inferred_start_time=inferred_start_time,
        computed_distance_m=computed_distance,
        computed_duration_s=computed_duration,
        duplicate_reason=duplicate_reason,
        parse_status=status,
        points_raw_count=raw,
        points_normalized_count=norm,
        points_cleaned_count=cleaned,
        fields_detected_json=fields,
        fields_dropped_json=dropped,
...
def placeholder_metadata(filename: str) -> ActivityImportMetadata:
    return ActivityImportMetadata(
        ...
        title=strip_activity_suffix(filename),
        description=None,
        sport_type="Run",
        filename=filename,
...
def metadata_from_manual_file(
    path: Path,
    original_filename: str,
    points: list[ParsedTrackPoint],
    cleaned,
    default_tz: str,
) -> ActivityImportMetadata:
    ...
    return ActivityImportMetadata(
        source_activity_id=None,
        title=inferred_title or strip_activity_suffix(original_filename),
        description=None,
        sport_type="Run",
        ...
        elevation_gain_m=elevation_gain_m(cleaned),
        avg_speed_mps=(comp / moving if comp and moving else None),
        max_speed_mps=max([p.speed_mps or 0 for p in points], default=None),
        avg_heart_rate_bpm=mean(p.heart_rate_bpm for p in cleaned),
        max_heart_rate_bpm=max(
            [p.heart_rate_bpm for p in cleaned if p.heart_rate_bpm is not None],
            default=None,
        ),
        avg_cadence_spm=mean(p.cadence_spm for p in cleaned),
        warnings=warnings,
    )
...
def find_fuzzy_duplicate(
    s: Session, meta: ActivityImportMetadata, comp_distance: float | None
) -> tuple[Activity | None, str | None]:
    start = comparable_dt(meta.start_time_utc or meta.start_time_local)
    dist = (
        meta.source_distance_m if meta.source_distance_m is not None else comp_distance
    )
    ...
    for activity in s.exec(select(Activity)).all():
        ...
        other_dist = (
            ...
            if activity.source_distance_m is not None
            else activity.computed_distance_m
        )
        other_dur = (
            activity.moving_time_s
            if activity.moving_time_s is not None
            else activity.elapsed_time_s
        )
        if other_start is None or other_dist is None or other_dur is None:
            continue
        if (
            abs((other_start - start).total_seconds()) <= 60
            and abs(other_dist - dist) <= 50
            and abs(other_dur - dur) <= 60
        ):
...
def import_single_activity_file(
    ...
    *,
    ...
    metadata: ActivityImportMetadata | None = None,
    force_reprocess: bool = False,
    delete_after_success: bool = False,
    allow_fuzzy_dedupe: bool = True,
) -> str:
    original_filename = original_filename or path.name
    hash_ = file_sha256(path)
    key = (
        fallback_dedupe_key(metadata)
        if metadata and not metadata.source_activity_id
        else None
    )
    ...
    with Session(engine) as s:
        ...
        if metadata and metadata.source_activity_id:
            existing = s.exec(
                select(Activity).where(
                    Activity.source_activity_id == metadata.source_activity_id
                )
            ).one_or_none()
        elif key:
            existing = s.exec(
                select(Activity).where(Activity.fallback_dedupe_key == key)
            ).one_or_none()
        if existing and existing.file_hash == hash_ and not force_reprocess:
            s.add(
                diag(
                    ...
                    file_hash=hash_,
                    duplicate_reason="unchanged_file_hash",
                    warnings=["unchanged_file_hash_skipped"],
                )
            )
            s.commit()
            return "skipped"
        if not existing and not (metadata and metadata.source_activity_id):
            hash_match = s.exec(
                select(Activity).where(Activity.file_hash == hash_)
            ).first()
            if hash_match and not force_reprocess:
                s.add(
                    diag(
                        ...
                        placeholder,
                        "skipped",
                        activity_id=hash_match.id,
                        file_hash=hash_,
                        duplicate_reason="file_hash",
                        warnings=[f"duplicate_file_hash:activity_id={hash_match.id}"],
                    )
                )
                s.commit()
                return "skipped"
    ...
    meta = metadata or metadata_from_manual_file(
        path, original_filename, points, cleaned, get_settings().default_timezone
    )
    meta.filename = original_filename
    warnings = list(meta.warnings) + list(clean_warnings)
    if (
        meta.source_distance_m
        and comp
        and abs(meta.source_distance_m - comp) / max(meta.source_distance_m, 1) > 0.1
    ):
        warnings.append("distance_mismatch_source_vs_computed")
    splits, split_warnings = generate_splits(cleaned)
    warnings += split_warnings
    efforts, effort_warnings = generate_best_efforts(cleaned)
    warnings += effort_warnings
    route = simplify_route(cleaned)
    if route is None:
        warnings.append("route_not_available")
    ...
    with Session(engine) as s:
        ...
        if meta.source_activity_id:
            existing2 = s.exec(
                select(Activity).where(
                    Activity.source_activity_id == meta.source_activity_id
                )
            ).one_or_none()
        elif key:
            existing2 = s.exec(
            ...
            if dup:
                s.add(
                    diag(
                        ...
                        len(points),
                        len(cleaned),
                        fields_detected(points),
                        dropped,
                        warnings + [reason],
                        file_hash=hash_,
                        inferred_title=inferred_title or meta.title,
                        inferred_start_time=inferred_start,
                        computed_distance=comp,
                        computed_duration=meta.moving_time_s or meta.elapsed_time_s,
        ...
        try:
            if existing2:
                ...
                status = "reprocessed"
            distance_for_pace = (
                meta.source_distance_m if meta.source_distance_m is not None else comp
            )
            activity = Activity(
                source_activity_id=meta.source_activity_id,
                fallback_dedupe_key=key,
                source_filename=meta.filename,
                file_hash=hash_,
                title=meta.title,
                description=meta.description,
                source_sport_type=meta.sport_type,
                normalized_sport_type="run",
                ...
                computed_distance_m=comp,
                moving_time_s=meta.moving_time_s,
                elapsed_time_s=meta.elapsed_time_s,
                elevation_gain_m=meta.elevation_gain_m,
                avg_pace_s_per_km=(
                    meta.moving_time_s / (distance_for_pace / 1000)
                    if meta.moving_time_s and distance_for_pace
                    else None
                ),
                avg_speed_mps=meta.avg_speed_mps,
                max_speed_mps=meta.max_speed_mps,
                avg_heart_rate_bpm=meta.avg_heart_rate_bpm,
                max_heart_rate_bpm=meta.max_heart_rate_bpm,
                avg_cadence_spm=meta.avg_cadence_spm,
            ...
            for sp in splits:
                s.add(ActivitySplit(activity_id=activity.id, **sp))
            for ef in efforts:
                s.add(BestEffort(activity_id=activity.id, **ef))
            if route:
                s.add(ActivityRoute(activity_id=activity.id, **route))
            s.add(
                diag(
                    job_id,
                    meta,
...
def import_one_activity(
    ...
    force_reprocess_extensions: set[str] | None = None,
) -> str:
    path = resolve_activity_file(root, row.filename)
    meta = metadata_from_strava_row(row)
    if not path:
        with Session(engine) as s:
            s.add(diag(job_id, meta, "failed", error="activity file missing"))
            s.commit()
        raise FileNotFoundError(f"activity file missing: {row.filename}")
    ext = suffix_key(path).removesuffix(".gz").lstrip(".")
    force_reprocess = force_reprocess_all or ext in (
        force_reprocess_extensions or set()
    )
    return import_single_activity_file(
        job_id,
        path,
        original_filename=row.filename or path.name,
        metadata=meta,
        force_reprocess=force_reprocess,
        allow_fuzzy_dedupe=True,
    )
...
def process_import_job(
    ...
    try:
        ...
        rows, csv_warnings = read_activities_csv(
            root / "activities.csv", settings.default_timezone
        )
        ...
        for row in rows:
            if row.sport_type not in RUN_SPORT_TYPES:
                logger.info(
                    "Skipped activity: %s",
                    row.title,
                    extra={"sport_type": row.sport_type},
                )
                inc_job(job_id, skipped_non_run_activities_count=1)
                continue
            try:
                result = import_one_activity(
                    job_id, row, root, force_reprocess_all, force_reprocess_extensions
                )
        ...
        set_job(
            job_id,
            status="failed",
            error_message="Import failed unexpectedly. Check backend logs for details.",
...
def process_activity_files_job(
    job_id: int,
    ...
    set_job(
        ...
        run_activities_seen=len(file_paths),
    )
    try:
        for i, path in enumerate(file_paths):
            ...
            try:
                result = import_single_activity_file(
                    job_id, path, original_filename=original, delete_after_success=True
                )
                inc = {"processed_count": 1}
                if result == "new":
                    inc["new_count"] = 1
                ...
                logger.exception(
                    "Activity file import failed", extra={"source_filename": original}
                )
                meta = placeholder_metadata(original)
                with Session(engine) as s:
                    s.add(diag(job_id, meta, "failed", error=str(e)))
                    s.commit()
                inc_job(job_id, processed_count=1, failed_count=1)
        set_job(job_id, status="completed", completed_at=datetime.now(timezone.utc))
    except Exception:
        logger.exception(
            "Activity files import job failed", extra={"import_job_id": job_id}
        )
        set_job(
            job_id,
            status="failed",
            error_message="Activity file import failed unexpectedly. Check backend logs for details.",
            completed_at=datetime.now(timezone.utc),
        )
    finally:
        try:
