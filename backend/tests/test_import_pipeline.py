...
from app.importer.parsers import (
    GpxTrackPointsParser,
    TcxTrackPointsParser,
    ParsedTrackPoint,
    suffix_key,
)
from app.importer.strava_csv import read_activities_csv
from app.importer.derive import (
    clean_points,
    computed_distance_m,
    generate_splits,
...
def test_gpx_and_tcx_parsing(tmp_path):
    ...
    make_gpx(gpx)
    make_tcx(tcx)
    gp = GpxTrackPointsParser().parse(gpx)
    tp = TcxTrackPointsParser().parse(tcx)
    assert (
        gp[0].timestamp
        and gp[0].lat == -37.0
        and gp[0].elevation_m == 10
        and gp[0].heart_rate_bpm == 120
        and gp[0].cadence_spm == 170
    )
    assert (
        tp[0].timestamp
        and tp[0].lon == 145.0
        and tp[0].elevation_m == 10
        and tp[0].heart_rate_bpm == 121
        and tp[0].cadence_spm == 172
    )
...
def test_cleaning_distance_splits_efforts_route():
    ...
    assert (
        "non_increasing_timestamp" in dropped
        and "gps_spike_impossible_speed" in dropped
    )
    assert computed_distance_m(cleaned) and computed_distance_m(cleaned) > 1500
    splits, _ = generate_splits(cleaned)
    assert splits and splits[0]["distance_m"] == 1000
    efforts, _ = generate_best_efforts(cleaned)
    assert any(e["distance_m"] == 400 for e in efforts)
    route = simplify_route(cleaned)
    assert (
        route
        and isinstance(route["simplified_points_json"][0], list)
        and len(route["simplified_points_json"][0]) == 3
    )
...
def test_dedupe_and_failure_policy(tmp_path):
    ...
    assert (
        job1.new_count == 2
        and job1.skipped_non_run_activities_count == 1
        and job1.failed_count == 0
    )
...
def test_manual_exact_file_hash_duplicate_skipped(tmp_path):
    ...
    assert import_single_activity_file(make_job(), p, original_filename=p.name) == "new"
    assert (
        import_single_activity_file(make_job(), p, original_filename=p.name)
        == "skipped"
    )
    with Session(engine) as s:
        assert len(s.exec(select(Activity)).all()) == 1
        diag = s.exec(
            select(ActivityImportDiagnostic).where(
                ActivityImportDiagnostic.parse_status == "skipped"
            )
        ).one()
...
def test_manual_fuzzy_duplicate_skipped_with_different_hash(tmp_path):
    ...
    assert (
        import_single_activity_file(make_job(), b, original_filename=b.name)
        == "skipped"
    )
    with Session(engine) as s:
        assert len(s.exec(select(Activity)).all()) == 1
        ...
        assert diag.duplicate_reason and diag.duplicate_reason.startswith(
            "fuzzy_duplicate"
        )
...
def test_manual_nearby_non_duplicate_imported(tmp_path):
    ...
    make_named_gpx(a)
    make_named_gpx(
        b, lon2="145.011", extra="<!-- different distance beyond tolerance -->"
    )
    assert import_single_activity_file(make_job(), a, original_filename=a.name) == "new"
...
def test_dashboard_stats_use_computed_distance_for_manual_imports(tmp_path):
    ...
    with Session(engine) as s:
        a = s.exec(select(Activity)).one()
        assert (
            a.source_distance_m is None
            and a.computed_distance_m
            and a.computed_distance_m > 700
        )
        summ = summary(s)
        ...
        assert (
            long_rows
            and long_rows[0]["longest_run_distance_m"] == a.computed_distance_m
        )
        distribution_rows = distance_distribution(s)
        assert (
            sum(row["distance_m"] for row in distribution_rows) == a.computed_distance_m
        )
