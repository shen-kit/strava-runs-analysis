...
def test_fit_missing_elevation_sentinel_becomes_none():
    ...
    pts = parser.normalize(
        [{"timestamp": datetime(2026, 1, 1), "enhanced_altitude": -1, "altitude": -1}]
    )
        ...
        [
            {
                ...
                "elapsed_time_s": i * 30,
                "distance_m": i * 25,
                "elevation_m": (100 + i if i % 2 == 0 else None),
            }
            for i in range(8)
        ]
    )
...
def test_elevation_stream_omits_minus_one_sentinel_and_all_missing_has_boundaries_only():
    ...
    res = (
        TestClient(app)
        .get(f"/activities/{aid}/streams?types=elevation,heart_rate")
        .json()
    )
...
def test_stream_sparse_sensor_points_use_interpolated_distance():
    ...
    aid = insert_activity(
        [
            ...
            {
                "timestamp": t + timedelta(seconds=6),
                "elapsed_time_s": 6,
                "distance_m": None,
                "elevation_m": 52,
            },
    ...
    res = (
        TestClient(app)
        .get(f"/activities/{aid}/streams?types=heart_rate,cadence,elevation")
        .json()
    )
