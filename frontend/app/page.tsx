...
import { api, type BestEffortDistanceSetting, type PersonalBest, type StatRow, type TotalRow, type Zone } from "@/src/lib/api";
...
function zoneKey(i: number) { return `zone_${i}`; }
function minuteTick(v: number) { return `${Math.round(v)}m`; }
function minutesDuration(v: unknown) { const total = Math.max(0, Math.round(n(v) * 60)); return `${Math.floor(total / 60)}:${String(total % 60).padStart(2, "0")}`; }
...
function fillZoneRows(rows: StatRow[] = [], keys: string[], zones: Zone[], fallbackKey?: string) { const by = new Map(rows.map((r) => [String(r.bucket), r])); return keys.map((k) => ({ bucket:k, ...Object.fromEntries(zones.map((_, i) => [zoneKey(i), n(by.get(k)?.[zoneKey(i)])])), ...(fallbackKey ? { [fallbackKey]: n(by.get(k)?.[fallbackKey]) } : {}) })); }
...
export default function Home() {
  ...
  const heartRateZones = useQuery({ queryKey: ["stats", "zone-distribution", "heart_rate", bucket], queryFn: () => api.zoneDistribution("heart_rate", bucket) });
  const paceZones = useQuery({ queryKey: ["stats", "zone-distribution", "pace", bucket], queryFn: () => api.zoneDistribution("pace", bucket) });
  ...
  const heartRateZoneRows = useMemo(() => fillZoneRows(heartRateZones.data, keys, settings.trainingZones.heartRate, "unknown"), [heartRateZones.data, keys, settings.trainingZones.heartRate]);
  const paceZoneRows = useMemo(() => fillZoneRows(paceZones.data, keys, settings.trainingZones.pace, "outside"), [paceZones.data, keys, settings.trainingZones.pace]);
  ...
  return <main className="page-shell page-stack">
    ...
    {visible.summary && (summary.isLoading ? <Loading text="Loading summary…" /> : summary.isError ? <ErrorState text="Summary failed to load." /> : <section className="dashboard-summary"><Card label="Runs" value={s?.total_runs ?? 0}/><Card label="Distance" value={formatDistance(s?.total_distance_m)}/><Card label="Duration" value={formatDuration(s?.total_moving_time_s)}/><Card label="Elevation" value={formatElevation(s?.total_elevation_gain_m)}/><Card label="Avg pace" value={formatPace(s?.average_pace_s_per_km)}/><Card label="Longest" value={formatDistance(s?.longest_run_distance_m)}/><Card label="This month" value={formatDistance(s?.current_month_distance_m)}/><Card label="This year" value={formatDistance(s?.current_year_distance_m)}/></section>)}

    <section className="dashboard-chart-grid grid gap-4 xl:grid-cols-2">
      ...
      {visible.heartRateZones && <ChartSection title="Heart-rate zone time" subtitle="Minutes spent in each heart-rate zone by period."><ZoneStack data={heartRateZoneRows} zones={settings.trainingZones.heartRate} fallback={{ key: "unknown", label: "Unknown" }}/></ChartSection>}
      {visible.paceZones && <ChartSection title="Pace zone time" subtitle="Minutes spent in each pace zone by period."><ZoneStack data={paceZoneRows} zones={settings.trainingZones.pace} fallback={{ key: "outside", label: "Outside zones" }}/></ChartSection>}
...
function ZoneStack({ data, zones, fallback }: { data: ChartRow[]; zones: Zone[]; fallback?: { key: string; label: string } }) { if (!data.length || !zones.length) return <Empty/>; return <ChartWrap><BarChart data={data}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="bucket" label={{value:"Period", position:"insideBottom", offset:-4}}/><YAxis tickFormatter={minuteTick} label={{value:"Minutes", angle:-90, position:"insideLeft"}}/><Tooltip content={<ZoneTooltip fallbackKey={fallback?.key}/>} />{zones.map((z, i) => <Bar key={`${z.label}-${i}`} dataKey={zoneKey(i)} name={z.label} stackId="zones" fill={palette[i % palette.length]} radius={!fallback && i === zones.length - 1 ? [4,4,0,0] : [0,0,0,0]}/>) }{fallback && <Bar dataKey={fallback.key} name={fallback.label} stackId="zones" fill={C.muted} radius={[4,4,0,0]}/>}</BarChart></ChartWrap>; }
function zoneTooltipRank(dataKey: unknown, fallbackKey?: string) { const key = String(dataKey); if (fallbackKey && key === fallbackKey) return Number.MAX_SAFE_INTEGER; const match = key.match(/^zone_(\d+)$/); return match ? Number(match[1]) : -1; }
function ZoneTooltip({ active, payload, label, fallbackKey }: { active?: boolean; payload?: { name?: ReactNode; value?: unknown; color?: string; dataKey?: unknown }[]; label?: ReactNode; fallbackKey?: string }) { if (!active || !payload?.length) return null; const rows = [...payload].sort((a, b) => zoneTooltipRank(b.dataKey, fallbackKey) - zoneTooltipRank(a.dataKey, fallbackKey)); return <div className="recharts-default-tooltip"><p className="recharts-tooltip-label">{label}</p>{rows.map((p) => <div key={String(p.dataKey)} style={{ color: p.color }}><span>{p.name}: </span><span>{minutesDuration(p.value)}</span></div>)}</div>; }
function PersonalBests({ data, distances }: { data: PersonalBest[]; distances: BestEffortDistanceSetting[] }) { if (!data.length) return <Empty/>; return <><div className="table-wrap responsive-table-desktop"><table className="table"><thead><tr><th>Distance</th><th>Duration</th><th>Pace</th><th>Activity</th><th>Date</th></tr></thead><tbody>{data.map((pb) => <tr key={pb.distance_m}><td>{pb.label ?? effortLabel(pb.distance_m, distances)}</td><td>{formatDuration(pb.duration_s)}</td><td>{formatPace(pb.pace_s_per_km)}</td><td><Link href={`/activities/${pb.activity_id}`}>{pb.activity_title}</Link></td><td>{formatDate(pb.local_date)}</td></tr>)}</tbody></table></div><div className="mobile-card-list">{data.map((pb) => <article className="mobile-row-card" key={pb.distance_m}><div><div className="mobile-row-title">{pb.label ?? effortLabel(pb.distance_m, distances)}</div><Link href={`/activities/${pb.activity_id}`} className="mobile-row-subtitle">{pb.activity_title}</Link></div><div className="mobile-row-metrics"><span><b>Duration</b>{formatDuration(pb.duration_s)}</span><span><b>Pace</b>{formatPace(pb.pace_s_per_km)}</span><span><b>Date</b>{formatDate(pb.local_date)}</span></div></article>)}</div></>; }
function RecentRuns({ data }: { data: { id: number; title: string; local_date: string; source_distance_m?: number | null; computed_distance_m?: number | null; moving_time_s?: number | null; elapsed_time_s?: number | null; avg_pace_s_per_km?: number | null; elevation_gain_m?: number | null; avg_heart_rate_bpm?: number | null; avg_cadence_spm?: number | null }[] }) { if (!data.length) return <Empty/>; return <><div className="table-wrap responsive-table-desktop"><table className="table"><thead><tr><th>Date</th><th>Title</th><th>Distance</th><th>Duration</th><th>Pace</th><th>Elev</th><th>HR</th><th>Cadence</th></tr></thead><tbody>{data.map((a) => <tr key={a.id}><td>{formatDate(a.local_date)}</td><td><Link href={`/activities/${a.id}`}>{a.title}</Link></td><td>{formatDistance(a.source_distance_m ?? a.computed_distance_m)}</td><td>{formatDuration(a.moving_time_s ?? a.elapsed_time_s)}</td><td>{formatPace(a.avg_pace_s_per_km)}</td><td>{formatElevation(a.elevation_gain_m)}</td><td>{a.avg_heart_rate_bpm ? Math.round(a.avg_heart_rate_bpm) : "—"}</td><td>{a.avg_cadence_spm ? Math.round(a.avg_cadence_spm) : "—"}</td></tr>)}</tbody></table></div><div className="mobile-card-list">{data.map((a) => <article className="mobile-row-card" key={a.id}><div><Link href={`/activities/${a.id}`} className="mobile-row-title">{a.title}</Link><div className="mobile-row-subtitle">{formatDate(a.local_date)}</div></div><div className="mobile-row-metrics"><span><b>Distance</b>{formatDistance(a.source_distance_m ?? a.computed_distance_m)}</span><span><b>Duration</b>{formatDuration(a.moving_time_s ?? a.elapsed_time_s)}</span><span><b>Pace</b>{formatPace(a.avg_pace_s_per_km)}</span></div></article>)}</div></>; }
