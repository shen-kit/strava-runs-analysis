...
export default function ActivityDetailPage() {
  ...
  return <main className="mx-auto max-w-6xl space-y-6 p-4">{!a ? <p>Loading…</p> : <>
    ...
    <section className="card"><h2 className="mb-3 text-xl font-semibold">Route</h2><RouteMap activityId={id} points={route.data?.simplified_points_json ?? []}/><p className="mt-2 text-sm text-slate-500">{route.data?.simplified_point_count ?? 0}/{route.data?.original_point_count ?? 0} points, tolerance {route.data?.simplification_tolerance_m ?? "—"} m</p></section>
    ...
    <section className="grid gap-4 lg:grid-cols-2"><div className="card"><h2 className="mb-2 text-lg font-semibold">Pace</h2><StreamLine data={streamRows("pace", "pace", (v) => v / 60)} yKey="pace" name="min/km" kind="pace" noData="No pace data" xDomainKm={xDomainKm} elevationData={elevationRows} /></div><div className="card"><h2 className="mb-2 text-lg font-semibold">Elevation</h2><StreamLine data={elevationRows} yKey="elevation" name="m" kind="elevation" noData="No elevation data" xDomainKm={xDomainKm} /></div><div className="card"><h2 className="mb-2 text-lg font-semibold">Heart rate</h2><StreamLine data={streamRows("heart_rate", "heart_rate")} yKey="heart_rate" name="bpm" xDomainKm={xDomainKm} elevationData={elevationRows} /></div><div className="card"><h2 className="mb-2 text-lg font-semibold">Cadence</h2><StreamLine data={streamRows("cadence", "cadence")} yKey="cadence" name="spm" xDomainKm={xDomainKm} elevationData={elevationRows} /></div></section>
  </>}</main>;
}
