...
export default function ActivityDetailPage() {
  ...
  const streamRows = (name: string, valueKey: string) => (streams.data?.streams?.[name] ?? []).map(([d, v]: [number, number]) => ({ km: Number((d/1000).toFixed(2)), [valueKey]: Math.round(v * 10) / 10 }));
