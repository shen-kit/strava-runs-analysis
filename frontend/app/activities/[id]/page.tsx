...
export default function ActivityDetailPage() {
  ...
  const streamRows = (name: string, valueKey: string, transform: (v: number) => number = (v) => v) => (streams.data?.streams?.[name] ?? []).map(([d, v]: [number, number]) => ({ km: d / 1000, [valueKey]: transform(v) }));
  return <main className="mx-auto max-w-6xl space-y-6 p-4">{!a ? <p>Loading…</p> : <>
    ...
    <section className="grid gap-4 lg:grid-cols-2"><div className="card"><h2 className="mb-2 text-lg font-semibold">Pace</h2><StreamLine data={streamRows("pace", "pace", (v) => v / 60)} yKey="pace" name="min/km" kind="pace" noData="No pace data" /></div><div className="card"><h2 className="mb-2 text-lg font-semibold">Elevation</h2><StreamLine data={streamRows("elevation", "elevation")} yKey="elevation" name="m" kind="elevation" noData="No elevation data" /></div><div className="card"><h2 className="mb-2 text-lg font-semibold">Heart rate</h2><StreamLine data={streamRows("heart_rate", "heart_rate")} yKey="heart_rate" name="bpm" /></div><div className="card"><h2 className="mb-2 text-lg font-semibold">Cadence</h2><StreamLine data={streamRows("cadence", "cadence")} yKey="cadence" name="spm" /></div></section>
  </>}</main>;
}
